#!/usr/bin/env python3
"""GRPO training of Qwen2.5-7B-Instruct — designed for `hf jobs run`.
...
Alternative flavors:
    ...
    --flavor l4x1       ($0.80/hr) — recommended, comfortable for 7B
...
After it finishes, the trained adapter is pushed to:
    Itachi-42/disclosure-game-qwen-7b-grpo
...
os.environ["PRIVACY_GAME_TRAJECTORY_DIR"] = str(WORKSPACE / "privacy_game" / "outputs" / "trajectories_7b")
...
OUT_DIR = "outputs/grpo_7b"
ADAPTER_DIR = "outputs/grpo_adapter_7b"
HF_REPO = "Itachi-42/disclosure-game-qwen-7b-grpo"
...
training_args = GRPOConfig(
    ...
    max_completion_length=150,    # ↓ from 200 — shorter generation phase
    num_generations=2,            # ↓ from 4 — halves GPU work per step
    temperature=0.9,
    beta=0.04,
    max_steps=200,
    logging_steps=1,
...
METRICS_PATH = Path("outputs/metrics/grpo_run_7b.jsonl")
...
print(f"🚀 Training Qwen2.5-7B for 200 steps — expect ~70-90 min on L4", flush=True)
trainer.train()
...
print("=== TRAINED 7B ===", flush=True)
base = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.float16, device_map="cuda")
trained_7b = PeftModel.from_pretrained(base, ADAPTER_DIR).merge_and_unload()
trained_rewards = _eval(trained_7b, "trained_7b")
del trained_7b, base
gc.collect(); torch.cuda.empty_cache()

print("=== BASE 7B (untrained) ===", flush=True)
base_7b = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.float16, device_map="cuda")
base_rewards = _eval(base_7b, "base_7b")

delta = statistics.mean(trained_rewards) - statistics.mean(base_rewards)
print(f"\n📊 7B Δ = {delta:+.4f}", flush=True)

eval_path = Path("outputs/metrics/grpo_7b_eval.json")
eval_path.write_text(json.dumps({
    "n_eval": 50,
    "model_id": MODEL_ID,
    "trained_7b_mean": statistics.mean(trained_rewards),
    "trained_7b_std": statistics.stdev(trained_rewards),
    "base_7b_mean": statistics.mean(base_rewards),
    "base_7b_std": statistics.stdev(base_rewards),
    "delta": delta,
    "trained_7b_rewards": trained_rewards,
    "base_7b_rewards": base_rewards,
}, indent=2))
...
print("\n══════════════════════════════════════════════════════════════════")
print(f"   7B training complete: Δ = {delta:+.4f}")
