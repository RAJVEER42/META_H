#!/usr/bin/env python3
"""GRPO training of Qwen2.5-1.5B-Instruct — designed for `hf jobs run`.
...
Alternative flavors:
    ...
    --flavor l4x1       ($0.80/hr) — recommended, comfortable for 1.5B
...
After it finishes, the trained adapter is pushed to:
    Itachi-42/disclosure-game-qwen-1.5b-grpo
...
os.environ["PRIVACY_GAME_TRAJECTORY_DIR"] = str(WORKSPACE / "privacy_game" / "outputs" / "trajectories_15b")
...
OUT_DIR = "outputs/grpo_15b"
ADAPTER_DIR = "outputs/grpo_adapter_15b"
HF_REPO = "Itachi-42/disclosure-game-qwen-1.5b-grpo"
...
METRICS_PATH = Path("outputs/metrics/grpo_run_15b.jsonl")
...
print(f"🚀 Training Qwen2.5-1.5B for 200 steps — expect ~70-90 min on L4", flush=True)
trainer.train()
...
print("=== TRAINED 1.5B ===", flush=True)
base = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.float16, device_map="cuda")
trained_15b = PeftModel.from_pretrained(base, ADAPTER_DIR).merge_and_unload()
trained_rewards = _eval(trained_15b, "trained_1.5b")
del trained_15b, base
gc.collect(); torch.cuda.empty_cache()

print("=== BASE 1.5B (untrained) ===", flush=True)
base_15b = AutoModelForCausalLM.from_pretrained(MODEL_ID, torch_dtype=torch.float16, device_map="cuda")
base_rewards = _eval(base_15b, "base_1.5b")

delta = statistics.mean(trained_rewards) - statistics.mean(base_rewards)
print(f"\n📊 1.5B Δ = {delta:+.4f}", flush=True)

eval_path = Path("outputs/metrics/grpo_15b_eval.json")
eval_path.write_text(json.dumps({
    "n_eval": 50,
    "model_id": MODEL_ID,
    "trained_15b_mean": statistics.mean(trained_rewards),
    "trained_15b_std": statistics.stdev(trained_rewards),
    "base_15b_mean": statistics.mean(base_rewards),
    "base_15b_std": statistics.stdev(base_rewards),
    "delta": delta,
    "trained_15b_rewards": trained_rewards,
    "base_15b_rewards": base_rewards,
}, indent=2))
...
print("\n══════════════════════════════════════════════════════════════════")
print(f"   1.5B training complete: Δ = {delta:+.4f}")
